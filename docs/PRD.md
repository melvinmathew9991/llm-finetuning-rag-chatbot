...
- Answers grounded in retrieved chunks (Chroma vector store), returned with
  similarity-scored source chunks

## Out of scope (current version)

- Real inventory/pricing integration (no live product database or API)
- Multi-turn conversational memory (each question is currently answered independently)
...
- Notebook cells run top-to-bottom without error and reproduce the documented
  ROUGE deltas (currently blocked — see `Phases.md` Phase 1)
